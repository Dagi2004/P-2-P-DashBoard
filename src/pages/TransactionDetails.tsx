            ...
            <div className="md:p-6 p-3 bg-white shadow rounded-lg md:max-w-5xl max-w-3xl mx-auto mt-6">
                {transactiondetail ? (
                    <div className='text-black'> 
                        ...
                        <p className="mb-2"><strong className="md:text-xl text-md">Status:</strong> 
                            <span className={`px-2 py-1 rounded ${transactiondetail.status === "Completed" ? "bg-green-500" : transactiondetail.status === "Pending" ? "bg-yellow-500" : "bg-red-500"}`}>
                                {transactiondetail.status}
                            </span>
                        </p>
                        <p className="mb-2"><strong className="md:text-xl text-md">Transaction Type:</strong> {transactiondetail.transactionType}</p>
                        <p className="mb-2"><strong className="md:text-xl text-md">Payment Method:</strong> {transactiondetail.paymentMethod}</p>
                        <p className="mb-2"><strong className="md:text-xl text-md">Timestamp:</strong> {new Date(transactiondetail.timestamp).toLocaleString()}</p> {/* Format timestamp */}
                    </div>
                ) : (
                    <p className='md:p-4 p-2 bg-red-500 text-white font-bold text-3xl md:max-w-3xl max-w-xl'>
